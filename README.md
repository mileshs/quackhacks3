...
Copy `.env.example` values into your shell if you need to override the default Cloudflare account, Worker, or D1 names.

```powershell
$env:CLOUDFLARE_ACCOUNT_ID = "a95007cb065e2bfced646f55bfc5dd35"
$env:CLOUDFLARE_WORKER_NAME = "quackhacks3"
$env:CLOUDFLARE_D1_DATABASE_NAME = "quackhacks3"
```

## Notes For Next Steps

- The frontend currently has scaffold routes plus a global active-game proof over WebSockets.
- The pose test starts a webcam stream and lazy-loads MediaPipe, but it does not run full pose landmark detection yet.
- The saboteur page can broadcast temporary pose updates through the Durable Object WebSocket.
- The leaderboard writes to Cloudflare D1 through the typed Hono API.
